...
import { getImages } from '../../Redux/ProductReducer/action';

function ImageDropper({ onClose }: { onClose: () => void }) {
  const [images, setImages] = useState<any[]>([]);
  const reduxstoreImages = useSelector((selector: any) => selector.ProductReducer.Images);
  const dispatch = useDispatch()
  const [isUploading, setUploading] = useState(false)

  const handleImageDropper = (acceptedFiles: any[]) => {
  ...
  const uploadImages = () => {
    ...
    setUploading(true)
    axios.post(`${process.env.REACT_APP_SERVER_URL}/image-upload`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    })
      .then((res) => {
        dispatch(getImages(res.data.files))
        setUploading(false)
  ...
  useEffect(() => {
    localStorage.setItem("mailImages", JSON.stringify(reduxstoreImages))
  }, [reduxstoreImages])
  ...
  return (
    <div>
      <span className="upload-badge" style={{ display: "inline-block", padding: "2px 8px", backgroundColor: "#fed7d7", color: "#c53030", borderRadius: "4px", fontSize: "12px", marginBottom: "8px" }}>
        *Max 10 images in one Go
      </span>
      <Dropzone onDrop={handleImageDropper} multiple={true}>
